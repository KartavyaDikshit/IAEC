...
// Icons components using Bootstrap Icons (MIT License)
...
const SocialShareIcons: React.FC<SocialShareIconsProps> = ({ title, url }) => {
  ...
  const shareOnWhatsApp = () => {
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(title)}%20${encodeURIComponent(url)}`, '_blank');
  };
  ...
  const iconClasses = "w-6 h-6 text-gray-700 hover:text-gray-900 transition-colors duration-200"; // Tailwind classes for icons
  ...
  return (
    <div className="flex items-center mt-6 space-x-4">
      ...
      {showToast && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-md shadow-lg transition-opacity duration-300 z-50">
          Link copied!
        </div>
      )}
    </div>
